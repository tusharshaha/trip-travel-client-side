...
const ManageOrder = () => {
  ...
  useEffect(() => {
    fetch("https://trip-travel.vercel.app/myOrder/all")
      .then((res) => res.json())
  ...
  const handleDelete = (id) => {
    swal({
      ...
      if (willDelete) {
        axios
          .delete(`https://trip-travel.vercel.app/myOrder/${id}`)
          .then((res) => {
  ...
  const handleApprove = (id) => {
    axios.put(`https://trip-travel.vercel.app/myOrder/${id}`).then((res) => {
      swal("Good job!", "Successfuly Approved", "success");
      setUpdate(!update);
    }).catch(()=>swal("Opps!", "Order isn't Approved", "warning"));
  };
  return (
    <>
      <Header></Header>
